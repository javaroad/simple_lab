...
public class DateUtil {
     ...
     * 判断一个目标字符串是否是日期类型。
     * <p>
     * 当且仅当目标字符串符合{@link DateUtil#DEFAULT_LONG_FORMAT}、 {@link DateUtil#DEFAULT_SHORT_FORMAT}两种格式时返回true，其他情况返回false。
     * 
     * <pre>
     * 示例如下：
     ...
     * 转换日期到字符串，并以默认格式进行返回。
     * <p>
     * 默认格式请参见{@link DateUtil#DEFAULT_LONG_FORMAT}；转换为自定义格式请参见 {@link DateUtil#toString(Date, String)}。
     * 
     * <pre>
     * 与以下调用的行为完全相同： 
     ...
     * <p>
     * 默认格式请参见{@link DateUtil#DEFAULT_LONG_FORMAT}；转换为自定义格式请参见 {@link DateUtil#toString(long, String)}。
     * 
     * <pre>
     * 与以下调用的行为完全相同： 
     ...
     * 转换日期格式字符串为日期实例。
     * <p>
     * 当字符串不符合默认格式{@link #DEFAULT_SHORT_FORMAT}或{@link #DEFAULT_LONG_FORMAT} 是返回null，反之转换成Date对象返回。
     * 
     * @param dateString 日期格式字符串。
     * @return 日期实例。
     * @exception NullPointerException 当dateString为空时抛出此异常。
     */
     ...
     * <p>
     * 此方法转换一个符合format格式的字符串到日期类型。当字符串不符合format格式转换结果将返回null。如果传入的字符串、 格式化format参数为null，则转换结果 同样也将返回null。
     * 
     * <pre>
     * Examples:
     * 
    ...
    /**
     * 通过指定毫秒数（以表示自从标准基准时间（称为“历元（epoch）”，即 1970 年 1 月 1 日 00:00:00 GMT）以来的指定毫秒数）得到一个日期实例。
     * <p>
     * 
     * @param millisecond 毫秒数，其他情况返回计算后的Date对象。
     * @return 日期实例。
     */
     ...
     * <p>
     * 此方法通过对给定的日期的年份进行操作来获得计算后的日期实例。如果传入的Date对象为null，则返回null。 如果传入的years大于10000或者小于等于传入日期的年份部分，
     ...
     * <p>
     * 此方法通过对给定的日期的月份进行操作来获得计算后的日期实例。如果传入的Date对象为null，则返回null。 如果传入的months大于10000或者小于-10000，
     ...
     * 添加或减去指定时间。
     * <p>
     * 这个方法根据传入的参数数量，依次添加或减去 年数、月数、天数、小时数、分钟数、秒数。当date参数为空时返回null，其他情况返回计算后的Date对象。
     * <p>
     * 此方法使用Calendar进行计算，为给定日期添加时间。is参数是动态数组，根据传入的长度对日期进行计算。当传入正数则在对应的时间域上添加时间， 传入负数则在对应的时间域上减去时间。
     * 此方法没有规定临界值，不同于DateUtils.addYear、DateUtils.addMonth。 一切临界值和计算公式都由Calendar处理。
     * 
     * <pre>
     * 示例如下：
     ...
     * @return String
     */
    public static String getDateByDayAsStr(int day) {
     ...
     * @return String
     */
    public static long getDateByDayStart(int day) {
        Calendar date = Calendar.getInstance();
        date.add(Calendar.DATE, -day);
        date.set(Calendar.HOUR_OF_DAY, 0);
        date.set(Calendar.MINUTE, 0);
    ...
    public static void main(String[] args) {
        DateTime datetime = DateTime.parse("11:30:10", DateTimeFormat.forPattern("HH:mm:ss"));
